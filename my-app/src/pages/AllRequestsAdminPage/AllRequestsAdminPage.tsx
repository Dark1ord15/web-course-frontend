...
const AllRequestsAdminPage = () => {
    ...
    const handleChangeStatus = async (requestId: number, Newstatus: string) => {
        try {
            await axios.put(
                `/api/travelrequests/change-status-moderator/${requestId}?status=${Newstatus}`,
                null, 
                {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
            ...
            <div style={{ margin: '3% 10% 0 10%' }}>
                ...
                {(!requests || requests.length === 0) ? <h1 className='small-h1' style={{ marginTop: '5%' }}>Нет данных, которые соответствуют фильтрам</h1> :
                    <Table striped bordered hover>
                        <thead>
                            <tr>
                                <th key={'User'}>Клиент</th>
                                <th key={'Requeststatus'}>Статус</th>
                                <th key={'Creationdate'}>Дата создания</th>
                                <th key={'Formationdate'}>Дата формирования</th>
                                <th key={'Complitiondate'}>Дата завершения</th>
                                <th key={'Moderator'}>Модератор</th>
                            ...
                            {Object.values(request).map((value, index) => {
                                ...
                                return excludedIndices.includes(index) ? null :
                                    timeRows.includes(index) ? <td key={index}>{formattedTime(value as string) as React.ReactNode}</td> :
                                        <td key={index}>
                                            {index === 6 && !value ? "Нет модератора" : // Если поле модератора пустое
                                                index === 7 && !value ? "Не оплачено" : // Если поле оплаты пустое
                                                    value as React.ReactNode}
                                        </td>;
                            })}
                            {request.Requeststatus === 'rejected' || request.Requeststatus === "completed" ?
                                <>
